...
def Check_Box(*arg):
    ...
    while n!=l:
        t += '''<div class="checkbox">
                                    <label>
                                        <input type="checkbox" name="'''+t.replace (" ", "_")+'''" value="'''+arg[n]+'''">'''+arg[n]+'''
                                    </label>
                                </div>'''
        n+=1
    html = '''
<div class="form-group">
<label>'''+arg[0].capitalize()+'''</label>'''+t+'''                        
                            </div>'''
    return html
...
def Radio_Button(*arg):
    ...
    while n!=l:
        t += '''<div class="radio">
                                    <label>
                                        <input type="radio" name="'''+t.replace (" ", "_")+'''" id="optionsRadios1" value="'''+arg[n]+'''" >'''+arg[n]+'''
                                </label>
                                </div>'''
        n+=1
    html = '''
...
def Select(*arg):
    ...
    t = ''
    while n!=l:
        t += '''<option>'''+arg[n]+'''</option>'''
        n+=1
    html = '''
<div class="form-group">
                                <label>'''+arg[0].capitalize()+'''</label>
                                <select class="form-control" name='''+t.replace (" ", "_")+'''>
...
<div class="form-group">
                                <label>'''+arg[0].capitalize()+'''</label>
                                <select multiple class="form-control" name='''+t.replace (" ", "_")+'''>
                        '''+t+'''
                                </select>
                            </div>
'''
...
def CreateHTML(name, html):
    f = open("/media/dhrubajyoti/Work/public_html/form_builder/admin/"+name.lower()+".php", "w")
    f.write(html)
    f.close()
